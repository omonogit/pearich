...
def home(request):
    ...
    out_of_stock=Products.objects.filter(quantity__lte=0).count()

    exipred=Products.objects.annotate(
...
def products(request):
    ...
    stocks = Products.objects.all().order_by("-id")


    ex=Products.objects.annotate(
    expired=ExpressionWrapper(Q(valid_to__lt=Now()), output_field=BooleanField())
    ).filter(expired=True)

    eo=Products.objects.annotate(
    expired=ExpressionWrapper(Q(valid_to__lt=Now()), output_field=BooleanField())
